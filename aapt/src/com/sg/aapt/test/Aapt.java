...
public class Aapt {
	...
	public static void main(String[] args) {
		try {
			...
			editClass("com.sg.a_test.MainActivity", "com.sg.a_test",mainPath, mainPath);
			Thread.sleep(5000);
			
			aapt.buildDex();
			Thread.sleep(3000);

			aapt.editManifest("com.sg.a_test", "com.sg.a_test.MainActivity");
			Thread.sleep(3000);
	...
	private void buildClass(){
		try {
			...
			buffer.append("cmd.exe /c start .\\makeClass.bat");
...
//			buffer.append("cmd.exe /c copy C:\\Users\\Vampire\\Desktop\\1.txt C:\\Users\\Vampire\\Desktop\\1");
			...
			//Process process = Runtime.getRuntime().exec(buffer.toString(), null, new File(Constants.DIR_APK_FILE));
			  ...
			  Runtime rt = Runtime.getRuntime();  
		        Process ps = null;  
		        try {  
		            ps = rt.exec(buffer.toString(), null, new File(Constants.DIR_APK_FILE));
		        ...
		        int i = ps.exitValue();  
		        if (i == 0) {  
		            System.out.println("编译class文件执行完成.");  
		        } else {  
		            System.out.println("编译class文件执行失败.");  
		        }  
		        ps.destroy();  
		        ps = null;  
...
//			new ProguardJump().proguardStart();
			...
			int dexTime = 0;
//			Thread.sleep(10000);
			while(!existFile(Constants.DIR_PROGUARD_LIB_ADD, "demo.jar")){
				Thread.sleep(1000);
				dexTime++;
			...
			buffer1.append("cmd.exe /c  start .\\makeDex.bat");
			...
			while(!existFile(Constants.DIR_PROGUARD_LIB_ADD, "classes.dex")){
				Thread.sleep(1000);
				dexTime++;
				System.out.println(dexTime);
			}
